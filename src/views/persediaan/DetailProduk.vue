      ...
      <div class="flex">
        ...
        <div class="flex flex-col w-2/3 ml-10 relative">
          <Transition name="slide-up">
            <div v-if="stateShow == 'insight'" class="bg-white shadow-md dark:bg-gray-800 w-full rounded-lg px-6 py-6 absolute">
            ...
            <div v-else-if="stateShow == 'price'" class="bg-white shadow-md dark:bg-gray-800 w-full rounded-lg px-6 py-6">
              ...
              <div class="flex flex-col justify-between">
                  ...
                  <table class="lg:w-full min-w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed table-striped">
                    <thead class="text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400 text-center">
                      <tr>
                        ...
                        <th scope="col" class="px-4 py-2 w-36 border border-slate-400 dark:border-slate-600">Harga Jual</th>
                      </tr>
                    </thead>
                    <tbody>
                      ...
                      <tr
                        ...
                        "
                      >
                        <td class="px-4 py-2 text-center">{{ itemPriceStore.from + index }}</td>
                        <td class="px-4 py-2">{{ moment(item.created_at).format('DD MMMM YYYY') }}</td>
                        <td class="px-4 py-2">{{ IDRCurrency.format(item.price) }}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                  <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                    Showing
                    <span class="font-semibold text-gray-900 dark:text-white">{{ itemPriceStore.from }} - {{ itemPriceStore.to }}</span>
                    of
                  ...
                  <ul class="inline-flex items-stretch -space-x-px">
                    ...
                    <li>
                      <a
                        @click="itemPriceStore.lastPage == itemPriceStore.currentPage ? '' : itemPriceStore.getData(nextPage)"
                        :class="
            ...
            <div v-else class="bg-white shadow-md dark:bg-gray-800 w-full rounded-lg px-6 py-6 absolute">
              ...
              <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">Dibawah ini merupakan data mutasi dari produk</p>
              <div class="flex flex-col justify-between">
                <div class="overflow-x-auto">
                  <table class="lg:w-full min-w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed table-striped">
                    <thead class="text-gray-700 uppercase bg-gray-100 dark:bg-gray-700 dark:text-gray-400 text-center">
                    ...
                    <tbody>
                      ...
                      <tr
                        ...
                        "
                      >
                        <td class="px-4 py-2 text-center">{{ itemMutationStore.from + index }}</td>
                        <td class="px-4 py-2">{{ moment(item.created_at).format('DD MMMM YYYY') }}</td>
                        <td class="px-4 py-2">{{ item.debit }}</td>
                        <td class="px-4 py-2">{{ item.credit }}</td>
                        <td class="px-4 py-2">
                          <router-link :to="item.link" v-if="item.link == null ? false : true">
                            <span class="text-blue-700 dark:text-blue-400"> {{ item.notes }} </span></router-link
                          >
                          <span v-else>
                            {{ item.notes }}
                          </span>
