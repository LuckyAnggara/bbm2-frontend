            ...
            <div class="flex flex-col space-y-6">
              <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                  ...
                  <tbody>
                    ...
                    >
                      <td class="px-4 py-3">{{ itemPriceStore.from + index }}</td>
                      <td class="px-4 py-3">{{ moment(item.created_at).format('DD MMMM YYYY') }}</td>
                      <td class="px-4 py-3">{{ IDRCurrency.format(item.price) }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4" aria-label="Table navigation">
                ...
                <ul class="inline-flex items-stretch -space-x-px">
                  <li>
                    <a
                      @click="itemPriceStore.getData(previousPage)"
                  ...
                  <li>
                    <a
                      @click="itemPriceStore.getData(nextPage)"
                      :disabled="itemPriceStore.lastPage == itemPriceStore.currentPage ? true : false"
                      :class="
